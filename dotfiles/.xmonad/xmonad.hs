...
myKeys conf@(XConfig {modMask = modMask}) = M.fromList $
  ...
  , ((modMask                              , xK_dollar), safeSpawn "alacritty" ["-e", "k9s"])
  ...
  , ((modMask .|. shiftMask .|. mod4Mask   , xK_Left  ), shiftToPrev >> prevWS)
  , ((modMask .|. shiftMask .|. mod4Mask   , xK_Right ), shiftToNext >> nextWS)
